 ...
 *      cal.destroy();
 *   }
 *   var cal = new Calendar(1, new Date(), selFunc, closeFunc);
 * \endcode
 *
 ...
 * calendar using Calendar.create().
 *
...
Calendar = function (firstDayOfWeek, dateStr, onSelected, onClose) {
	...
	this.minMonth = 0;
	this.maxMonth = 11;
	this.dateFormat = Calendar.i18n("DEF_DATE_FORMAT");
	this.ttDateFormat = Calendar.i18n("TT_DATE_FORMAT");
	this.historyDateFormat = "%B %d, %Y";
	this.isPopup = true;
	this.weekNumbers = true;
	this.noGrab = false;
	if (Calendar.prefs.fdow || (Calendar.prefs.fdow == 0)) {
		this.firstDayOfWeek = parseInt(Calendar.prefs.fdow, 10);
	}
	else {
		var fd = 0;
		if (typeof firstDayOfWeek == "number") {
			fd = firstDayOfWeek;
		} else if (typeof Calendar._FD == 'number') {
			fd = Calendar._FD;
		}
 ...
 */
Calendar._initSDN = function() {
	if (typeof Calendar._TT._SDN == "undefined") {
		// table of short day names
		if (typeof Calendar._TT._SDN_len == "undefined")
			Calendar._TT._SDN_len = 3;
		var ar = [];
		for (var i = 8; i > 0;) {
			ar[--i] = Calendar._TT._DN[i].substr(0, Calendar._TT._SDN_len);
		}
		Calendar._TT._SDN = ar;
		// table of short month names
		if (typeof Calendar._TT._SMN_len == "undefined")
			Calendar._TT._SMN_len = 3;
		ar = [];
		for (var i = 12; i > 0;) {
			ar[--i] = Calendar._TT._MN[i].substr(0, Calendar._TT._SMN_len);
		}
		Calendar._TT._SMN = ar;
	}
};
 ...
 */
Calendar.i18n = function(str, type) {
	var tr = '';
	if (!type) {
		// normal _TT request
		if (Calendar._TT)
			tr = Calendar._TT[str];
		if (!tr && Calendar._TT_en)
			tr = Calendar._TT_en[str];
	} else switch(type) {
	    case "dn"  : tr = Calendar._TT._DN[str];  break;
	    case "sdn" : tr = Calendar._TT._SDN[str]; break;
	    case "mn"  : tr = Calendar._TT._MN[str];  break;
	    case "smn" : tr = Calendar._TT._SMN[str]; break;
	}
	if (!tr) tr = "" + str;
	return tr;
};
...
/// preferences
Calendar.prefs = {
	fdow     : null,	/**< when NULL we will use the options passed at Calendar.setup */
 ...
 */
Calendar.savePrefs = function() {
	// FIXME: should we make the domain, path and expiration time configurable?
	// I guess these defaults are right though..
	Utils.writeCookie("DYN_CAL", Utils.makePref(this.prefs), null, '/', 30);
};

/**
 * Loads the preference cookie and merges saved prefs to Calendar.prefs.
 */
Calendar.loadPrefs = function() {
	var txt = Utils.getCookie("DYN_CAL"), tmp;
	if (txt) {
		tmp = Utils.loadPref(txt);
		if (tmp)
			Utils.mergeObjects(this.prefs, tmp);
	}
	// FIXME: DEBUG!
...
Calendar._add_evs = function(el) {
	var C = Calendar;
	Utils.addEvent(el, "mouseover", C.dayMouseOver);
	Utils.addEvent(el, "mousedown", C.dayMouseDown);
	Utils.addEvent(el, "mouseout", C.dayMouseOut);
	if (is_ie)
		Utils.addEvent(el, "dblclick", C.dayMouseDblClick);
};

/**
 * \internal This function undoes what Calendar._add_evs did, therefore
...
Calendar._del_evs = function(el) {
	var C = this;
	Utils.removeEvent(el, "mouseover", C.dayMouseOver);
	Utils.removeEvent(el, "mousedown", C.dayMouseDown);
	Utils.removeEvent(el, "mouseout", C.dayMouseOut);
	if (is_ie)
		Utils.removeEvent(el, "dblclick", C.dayMouseDblClick);
};
...
/** Similar to the above functions, but for the years combo. */
Calendar.find = function(el, field) {
	if (typeof eval("el."+field) != "undefined") {
		return el;
	} else if (el.parentNode && typeof eval("el.parentNode."+field) != "undefined") {
		return el.parentNode;
	}
	return null;
};
...
Calendar.showMonthsCombo = function () {
	...
	for (var i = mc.firstChild; i; i = i.nextSibling) {
		var m = i.month;
		Utils.removeClass(i, "hilite");
		Utils.removeClass(i, "active");
		Utils.removeClass(i, "disabled");
		i.disabled = false;
		...
		if (m == MM)
			Utils.addClass(cal.activeMonth = i, "active");
	}
	var s = mc.style;
	s.display = "block";
 ...
 * active calendar.
 */
Calendar.showHistoryCombo = function() {
	var cal = Calendar._C, a, h, i, cd, hc, s, tmp, div;
	if (!cal)
		return false;
	hc = cal.histCombo;
	while (hc.firstChild)
		hc.removeChild(hc.lastChild);
	if (Calendar.prefs.history) {
		a = Calendar.prefs.history.split(/,/);
		i = 0;
		while (tmp = a[i++]) {
			tmp = tmp.split(/\//);
			h = Utils.createElement("div");
			h.className = is_ie ? "label-IEfix" : "label";
...
Calendar.showYearsCombo = function (fwd) {
	...
	if (cal.hilitedYear) {
		Utils.removeClass(cal.hilitedYear, "hilite");
	}
	if (cal.activeYear) {
		Utils.removeClass(cal.activeYear, "active");
	}
	cal.activeYear = null;
...
/**
 * @author C1161151 - Rafael Veloso 
 * 
 * Mostra um combo com as Horas. 
 */
Calendar.showTimeCombo = function () {
	var cal = Calendar._C;
	if (!cal) {
		return false;
	}
	cal.hourCombo.style.display = "none";
	var cd = cal.activeDiv;
	var mc = (cd.navtypetime == 1)?cal.hourCombo:cal.minuteCombo;
	
	var date = cal.date, comparador = (cd.navtypetime == 1)? date.getHours() : date.getMinutes() ;
	for (var i = mc.firstChild; i; i = i.nextSibling) {
		var m = (cd.navtypetime == 1)? i.hour : i.minute ;
		Utils.removeClass(i, "hilite");
		Utils.removeClass(i, "active");
		if (m == comparador){
			Utils.addClass(cal.activeHour = i, "active");
		}
	}
	...
	var mcw = mc.offsetWidth;
	var posicao = (cd.navtypetime == 1)? (cal.dateFormat == "%H")? 120 : 81 :160;
	
	s.left = (posicao + cd.offsetWidth - mcw) + "px";
	s.top = (20 + cd.offsetHeight) + "px";
 ...
 *
 * Also, the Calendar.cellClick() function is called, which further
...
Calendar.tableMouseUp = function(ev) {
	...
	var target = Utils.getTargetElement(ev);
	ev || (ev = window.event);
	...
	mon = (mon)? mon : Calendar.find(target,"hour");
	mon = (mon)? mon : Calendar.find(target,"minute");
	var date = null;
	if (mon) {
		if (typeof mon.hour != "undefined") {
			date = new Date(cal.date);
			date.setHours(mon.hour);
			cal.date = date;
			cal.onSetTime();
			cal.dateClicked = true;
			cal.callHandler();
			if(cal.time24 && cal.activeHour != mon){
				Utils.removeClass(mon, "hilite");
				Utils.removeClass(cal.activeHour, "active");
				Utils.addClass(cal.activeHour = mon, "active");
				return Utils.stopEvent(ev);
			}
		} else if (typeof mon.minute != "undefined") {
			date = new Date(cal.date);
			date.setMinutes(mon.minute);
			cal.setDate(date);
			cal.dateClicked = false;
			cal.callHandler();
		} else if (!mon.disabled) {
			date = new Date(cal.date);
		...
		if (year) {
			...
			var hist = Calendar.findHist(target);
	...
	Utils.removeEvent(window.document, "mouseup", Calendar.tableMouseUp);
	Utils.removeEvent(window.document, "mouseover", Calendar.tableMouseOver);
	Utils.removeEvent(window.document, "mousemove", Calendar.tableMouseOver);
	cal._hideCombos();
	Calendar._C = null;
	return Utils.stopEvent(ev);
};
 ...
 * implemented by the Dynarch.com calendar).  Finally, it determines if the
...
Calendar.tableMouseOver = function (ev) {
	...
	var target = Utils.getTargetElement(ev);
	if (target == el || target.parentNode == el) {
		Utils.addClass(el, "hilite active");
		Utils.addClass(el.parentNode, "rowhilite");
	} else {
		if (typeof el.navtype == "undefined" ||
		    (el.navtype != 50 && ((el.navtype == 0 && !cal.histCombo) || Math.abs(el.navtype) > 2)))
			Utils.removeClass(el, "active");
		Utils.removeClass(el, "hilite");
		Utils.removeClass(el.parentNode, "rowhilite");
	}
	ev || (ev = window.event);
	/*
	if (el.navtype == 50 && target != el) {
		var pos = Utils.getAbsolutePos(el);
	...
	if (mon) {
		if (!mon.disabled) {
			if (mon.month != cal.date.getMonth()) {
				if (cal.hilitedMonth) {
					Utils.removeClass(cal.hilitedMonth, "hilite");
				}
				Utils.addClass(mon, "hilite");
		...
		if (year) {
			...
			if (hist) {
				if (!hist.histDate.dateEqualsTo(cal.date)) {
					if (cal.hilitedHist) {
						Utils.removeClass(cal.hilitedHist, "hilite");
					}
					Utils.addClass(hist, "hilite");
...
Calendar.tableMouseDown = function (ev) {
	if (Utils.getTargetElement(ev) == Utils.getElement(ev)) {
		return Utils.stopEvent(ev);
	}
};
 ...
 * Calendar.hideShowCovered() which repaints as appropriate any
...
Calendar.calDragEnd = function (ev) {
	...
	cal.dragging = false;
	Utils.removeEvent(window.document, "mousemove", Calendar.calDragIt);
	Utils.removeEvent(window.document, "mouseover", Calendar.calDragIt);
	Utils.removeEvent(window.document, "mouseup", Calendar.calDragEnd);
 ...
 * Calendar.tableMouseOver() event handler on the document.  If the
...
Calendar.dayMouseDown = function(ev) {
	...
	if (el.navtype != 300) {
		if (el.navtype == 50) {
			...
			Utils.addEvent(window.document, "mouseover", Calendar.tableMouseOver);
		} else
			Utils.addEvent(window.document, is_ie5 ? "mousemove" : "mouseover", Calendar.tableMouseOver);
		Utils.addClass(el, "hilite active");
		Utils.addEvent(window.document, "mouseup", Calendar.tableMouseUp);
	} else if (cal.isPopup) {
		cal._dragStart(ev);
	}
	...
	} else if (el.navtype == 0 && Calendar.prefs.history) {
		if (cal.timeout) clearTimeout(cal.timeout);
		cal.timeout = setTimeout("Calendar.showHistoryCombo()", 250);
	} else if (el.navtype == 50) {
		if (cal.timeout) clearTimeout(cal.timeout);
		cal.timeout = setTimeout("Calendar.showTimeCombo()", 250);
	} else {
		cal.timeout = null;
	}
	return Utils.stopEvent(ev);
...
Calendar.dayMouseDblClick = function(ev) {
	Calendar.cellClick(Utils.getElement(ev), ev || window.event);
	if (is_ie)
...
Calendar.dayMouseOver = function(ev) {
	...
	if (Utils.isRelated(el, ev) || Calendar._C || el.disabled) {
		return false;
	}
	if (el.ttip) {
...
Calendar.dayMouseOut = function(ev) {
	...
	if (el.caldate)
		Utils.removeClass(el.parentNode, "rowhilite");
	if (el.calendar)
		el.calendar.showHint((el.calendar.showsTime)?Calendar.i18n("SEL_TIME"):Calendar.i18n("SEL_DATE"));
	return Utils.stopEvent(ev);
};
...
Calendar.cellClick = function(el, ev) {
	...
	if (typeof el.navtype == "undefined") {
		if (cal.currentDateEl) {
			Utils.removeClass(cal.currentDateEl, "selected");
		...
		switch (el.navtype) {
		    case 400:
			Utils.removeClass(el, "hilite");
			var text = (!cal.params.campoMes)? Calendar.i18n("ABOUT") : Calendar.i18n("ABOUT_MES") ;
			if (typeof text != "undefined") {
				text = cal.showsTime ? Calendar._TT["ABOUT_TIME"] : text;
			} else {
				// FIXME: this should be removed as soon as lang files get updated!
				text = "Help and about box text is not translated into this language.\n" +
					"If you know this language and you feel generous please update\n" +
					"the corresponding file in \"lang\" subdir to match calendar-en.js\n" +
					"and send it back to <support@com> to get it into the distribution  ;-)\n\n" +
					"Thank you!\n" +
					"http://www.com\n";
			}
			alert(text);
			...
			Calendar.prefs.fdow = cal.firstDayOfWeek;
			Calendar.savePrefs();
			if (cal.onFDOW)
 ...
 * Calendar.prototype._init().  Therefore, by separating these 2
...
Calendar.prototype.create = function (_par) {
	...
	var table = Utils.createElement("table");
	this.table = table;
	table.cellSpacing = 0;
	table.cellPadding = 0;
	table.width = "203px";
	table.calendar = this;
	Utils.addEvent(table, "mousedown", Calendar.tableMouseDown);
	...
	var thead = Utils.createElement("thead", table);
	var cell = null;
	var row = null;

	var cal = this;
	var hh = function (text, cs, navtype) {
		cell = Utils.createElement("td", row);
		cell.colSpan = cs;
	...
	row = Utils.createElement("tr", thead);
	var title_length = 6;
	this.isPopup && --title_length;
	this.weekNumbers && ++title_length;

	var ajuda = hh("?", 1, 400).ttip = Calendar.i18n("INFO");
	this.title = hh("", title_length, 300);
	this.title.className = "title";
	
	if (this.isPopup) {
		this.title.ttip = Calendar.i18n("DRAG_TO_MOVE");
		this.title.style.cursor = "move";
		hh("&#x00d7;", 1, 200).ttip = Calendar.i18n("CLOSE");
	}
	if (this.showsTime) {
		ajuda = "14%";
		this.title.width = "72%";
		...
		if(!this.params.campoMes){
			row = Utils.createElement("tr", thead);
			row.className = "headrow";
		
			this._nav_py = hh("&#x00ab;", 1, -2);
			this._nav_py.ttip = Calendar.i18n("PREV_YEAR");
		
			this._nav_pm = hh("&#x2039;", 1, -1);
			this._nav_pm.ttip = Calendar.i18n("PREV_MONTH");
		
			this._nav_now = hh(Calendar.i18n("TODAY"), this.weekNumbers ? 4 : 3, 0);
			this._nav_now.ttip = Calendar.i18n("GO_TODAY");
		
			this._nav_nm = hh("&#x203a;", 1, 1);
			this._nav_nm.ttip = Calendar.i18n("NEXT_MONTH");
		
			this._nav_ny = hh("&#x00bb;", 1, 2);
			this._nav_ny.ttip = Calendar.i18n("NEXT_YEAR");
		}	
		// day names
		row = Utils.createElement("tr", thead);
		...
		if (this.weekNumbers) {
			...
			cell.appendChild(window.document.createTextNode(Calendar.i18n("WK")));
			var cal_wk = Calendar.i18n("WK")
				if (cal_wk == null) {
	...
	var tbody = Utils.createElement("tbody", table);
	this.tbody = tbody;

	for (i = 6; i > 0; --i) {
		row = Utils.createElement("tr", tbody);
		if (this.showsTime) row.style.display = "none";
		if (this.weekNumbers) {
			cell = Utils.createElement("td", row);
	...
	var tfoot = Utils.createElement("tfoot", table);

	if (this.showsTime) {
		row = Utils.createElement("tr", tfoot);
		row.className = "time";
		
		cell = Utils.createElement("td", row);
		cell.className = "timetext";
		cell.colSpan = this.weekNumbers ? 2 : 1;
		cell.innerHTML = Calendar.i18n("TIME") || "&nbsp;";
		...
		(function(){
			function makeTimePart(className, init, range_start, range_end) {
				...
				if (range_end) {
					...
					table = Utils.createElement("table", cell);
					table.cellSpacing = table.cellPadding = 0;
					if (className == "hour"){
						table.align = "right";
					}
					table.className = "calendar-time-scroller";
					tbody = Utils.createElement("tbody", table);
					tr    = Utils.createElement("tr", tbody);
					tr2   = Utils.createElement("tr", tbody);
				} else
					tr = row;
				part = Utils.createElement("td", tr);
				part.className = className;
				part.appendChild(window.document.createTextNode(init));
				part.calendar = cal;
				part.ttip = Calendar.i18n("TIME_PART");
				part.navtype = 50;
				part._range = [];
				if (!range_end)
					...
					var up = Utils.createElement("td", tr);
					up.className = "up";
					up.navtype = 201;
					up.calendar = cal;
					up.timePart = part;
					if (is_khtml)
						up.innerHTML = "&nbsp;";
					Calendar._add_evs(up);

					var down = Utils.createElement("td", tr2);
					down.className = "down";
					down.navtype = 202;
					down.calendar = cal;
					down.timePart = part;
			...
			H._step = 1;
			if(cal.dateFormat == "%H"){
				H.parentNode.parentNode.parentNode.align = "center";
				cell.colSpan = 5;
			} else {
				cell = Utils.createElement("td", row);
				cell.innerHTML = ":";
				...
				M = makeTimePart("minute", mins, 0, 59);
				//cell.style.visibility = "hidden";
				M.navtypetime = 2;
			...
			if (t12) {
				...
				Utils.createElement("td", row).innerHTML = "&nbsp;";
		...
		div = this.hourCombo = Utils.createElement("div", this.element);
		div.className = "combo";
		var contador = 13;
		var i = 1;
		if(cal.time24){
			Utils.addClass(div, "combohour");
			contador = 24;
			i = 0;
		}
		for ( ; i < contador; ++i) {
			var hr = Utils.createElement("div");
			hr.className = is_ie ? "label-IEfix" : "label";
			hr.hour = i;
			hr.appendChild(window.document.createTextNode(i));
			div.appendChild(hr);
		}
		
		div = this.minuteCombo = Utils.createElement("div", this.element);
		div.className = "combo";
		for (i = 0; i < 12; ++i) {
			var mn = Utils.createElement("div");
			mn.className = is_ie ? "label-IEfix" : "label";
			mn.minute = i*5;
			mn.appendChild(window.document.createTextNode((i*5)));
			div.appendChild(mn);
		}
	...
	cell = hh((this.showsTime)?Calendar.i18n("SEL_TIME"):Calendar.i18n("SEL_DATE"), this.weekNumbers ? 8 : 7, 300);
	cell.className = "ttip";
	if (this.isPopup) {
		cell.ttip = Calendar.i18n("DRAG_TO_MOVE");
		cell.style.cursor = "move";
	}
	this.tooltips = cell;

	div = this.monthsCombo = Utils.createElement("div", this.element);
	div.className = "combo";
	for (i = 0; i < 12; ++i) {
		var mn = Utils.createElement("div");
		mn.className = is_ie ? "label-IEfix" : "label";
		mn.month = i;
		mn.appendChild(window.document.createTextNode(Calendar.i18n(i, "smn")));
		div.appendChild(mn);
	}

	div = this.yearsCombo = Utils.createElement("div", this.element);
	div.className = "combo";
	for (i = 12; i > 0; --i) {
		var yr = Utils.createElement("div");
		yr.className = is_ie ? "label-IEfix" : "label";
		yr.appendChild(window.document.createTextNode(""));
		div.appendChild(yr);
	}

	div = this.histCombo = Utils.createElement("div", this.element);
...
Calendar._keyEvent = function(ev) {
	...
	var act = (is_ie || ev.type == "keypress");
	var K = ev.keyCode;
	if (ev.ctrlKey && (!cal.params.campoMes)) {
		switch (K) {
		    case 37: // KEY left
...
Calendar.prototype._init = function (firstDayOfWeek, date) {
	...
	var MN = Calendar.i18n(month, "smn");
	var ar_days = this.ar_days = [];
	var weekend = Calendar.i18n("WEEKEND");
		...
		for (; cell && (iday = date.getDate()); date.setDate(iday+1), cell = cell.nextSibling) {
			...
			if (!cell.disabled) {
				...
				if (dyear == TY && dmonth == TM && iday == TD) {
					...
					cell.ttip += Calendar.i18n("PART_TODAY");
				}
	...
	//this.title.firstChild.data = Calendar.i18n(month, "mn") + ", " + year;
	this.title.innerHTML = Calendar.i18n(month, "mn") + ", " + year;
	this.onSetTime();
...
Calendar.prototype._toggleMultipleDate = function(date) {
	if (this.multiple) {
		...
		if(this.params.sequencia){
			this.multiple = [];		
			var proximo = false;
			for(var i in this.datesCells){
				celula = this.datesCells[i];
				if((celula == cell) || proximo){
					if(contador == this.params.maximoDatas)
						proximo = false;
					else
						proximo = true;
					Utils.addClass(celula, "selected");
					this.multiple[i] = new Date(celula.caldate[0],celula.caldate[1],celula.caldate[2]);
					contador++;
				} else {
					Utils.removeClass(celula, "selected");
					delete this.multiple[i];
				}
			}
			contador--;
			var dias = 1;
			while(contador != this.params.maximoDatas){
				var indice = (celula.caldate[0]+""+(celula.caldate[1]+1)+""+dias);
				this.multiple[indice] = new Date(celula.caldate[0],(celula.caldate[1]+1),dias);
				contador++;dias++;
			}
		} else {
			for(var i in this.multiple)
			...
			if (cell) {
				...
				if (!d) {
					if(contador > this.params.maximoDatas)
						alert(Calendar.i18n("NUMERO_PERMITIDO")+this.params.maximoDatas);
					else{
...
Calendar.prototype.setDate = function (date) {
	...
	if (!date.equalsTo(this.date)) {
		...
		if (year == this.minYear && m < this.minMonth)
			this.showHint("<div class='error'>" + Calendar.i18n("E_RANGE") + " »»»</div>");
 ...
 */
Calendar.prototype.showHint = function(text) {
 ...
 * that keeps backwards compatibility with Dynarch.com calendar v0.9.6.
 *
 ...
 */
Calendar.prototype.updateHistory = function () {
	var a, i, d, tmp, s, str = "", len = Calendar.prefs.hsize - 1;
	if (Calendar.prefs.history) {
		a = Calendar.prefs.history.split(/,/);
		i = 0;
		while (i < len && (tmp = a[i++])) {
	...
	Calendar.prefs.history = this.date.print("%Y/%m/%d/%H/%M") + str;
	Calendar.savePrefs();
};
...
Calendar.prototype.destroy = function () {
	...
	Utils.destroy(this.element);
	Utils.destroy(this.WCH);
	Calendar._C = null;
...
Calendar._checkCalendar = function(ev) {
	...
	var el = is_ie ? Utils.getElement(ev) : Utils.getTargetElement(ev);
 ...
 */
Calendar.prototype.updateWCH = function(other_el) {
	Utils.setupWCH_el(this.WCH, this.element, other_el);
};
...
Calendar.prototype.show = function () {
	...
	for (var i = rows.length; i > 0;) {
		var row = rows[--i];
		Utils.removeClass(row, "rowhilite");
		var cells = row.getElementsByTagName("td");
		for (var j = cells.length; j > 0;) {
			var cell = cells[--j];
			Utils.removeClass(cell, "hilite");
			Utils.removeClass(cell, "active");
		}
	}
	...
	if (this.isPopup) {
		...
		if (!this.noGrab) {
			Utils.addEvent(window.document, "keydown", Calendar._keyEvent);
			Utils.addEvent(window.document, "keypress", Calendar._keyEvent);
			Utils.addEvent(window.document, "mousedown", Calendar._checkCalendar);
		}
	}
	this.hideShowCovered();
};
...
Calendar.prototype.hide = function () {
	if (this.isPopup) {
		Utils.removeEvent(window.document, "keydown", Calendar._keyEvent);
		Utils.removeEvent(window.document, "keypress", Calendar._keyEvent);
		Utils.removeEvent(window.document, "mousedown", Calendar._checkCalendar);
	}
	this.element.style.display = "none";
	Utils.hideWCH(this.WCH);
	this.hidden = true;
	this.hideShowCovered();
};
 ...
 * Calendar.prototype.setDateFormat() for a description of the \em str
 * format.
 *
...
Calendar.prototype.parseDate = function (str, fmt) {
	...
	if (!str){
		if(this.params.iniciar)
			str = this.params.iniciar;	
		else
...
Calendar.prototype._displayWeekdays = function () {
	...
	for (var i = 0; i < 7; ++i) {
		...
		if (i) {
			if (Calendar.i18n("DAY_FIRST") != null) {
				cell.ttip = Calendar.i18n("DAY_FIRST").replace("%s", Calendar.i18n(realday, "dn"));
			}
			cell.navtype = 100;
			cell.calendar = this;
			cell.fdow = realday;
		...
		if ((weekend != null) && (weekend.indexOf(realday.toString()) != -1)) {
						Utils.addClass(cell, "weekend");
		}
		cell.innerHTML = Calendar.i18n((i + fdow) % 7, "sdn");
		cell = cell.nextSibling;
	}
};
...
Calendar.prototype._hideCombos = function () {
	...
	if (this.showsTime) {
		this.hourCombo.style.display = "none";
		this.minuteCombo.style.display = "none";
	}
	this.updateWCH();
};
...
Calendar.prototype._dragStart = function (ev) {
	...
	Utils.addEvent(window.document, "mousemove", Calendar.calDragIt);
	Utils.addEvent(window.document, "mouseover", Calendar.calDragIt);
	Utils.addEvent(window.document, "mouseup", Calendar.calDragEnd);
};
...
Date.prototype.print = function (str) {
	...
	s["%a"] = Calendar.i18n(w, "sdn"); // abbreviated weekday name [FIXME: I18N]
	s["%A"] = Calendar.i18n(w, "dn"); // full weekday name
	s["%b"] = Calendar.i18n(m, "smn"); // abbreviated month name [FIXME: I18N]
	s["%B"] = Calendar.i18n(m, "mn"); // full month name
...
Date.parseDate = function (str, fmt) {
	...
	for (i = 0; i < a.length; ++i) {
		...
		switch (b[i]) {
		    ...
		    case "%B":
			for (j = 0; j < 12; ++j)
				if (Calendar.i18n(j, "mn").substr(0, a[i].length).toLowerCase() == a[i].toLowerCase()) {
					m = j;
					break;
				}
			break;
	...
	if(window.calendar && window.calendar.showsTime){
		var hora = new Date();
		hora.setHours(hr);
		hora.setMinutes(min);
		hora.setSeconds(sec);
	...
	if (y == 0 && m == -1 && d != 0)
		return new Date(new Date().getFullYear(), 0, d, hr, min, sec);
 ...
 * 
 * @return a new Tooltip object if called with "new Tooltip()"
 */
Tooltip = function(target, tooltip) {
	...
	this.timeout = null;
	Utils.addClass(tooltip, "tooltip");
	document.body.appendChild(tooltip);
	if (tooltip.title) {
		var title = Utils.createElement("div");
		tooltip.insertBefore(title, tooltip.firstChild);
		title.className = "title";
		title.innerHTML = unescape(tooltip.title);
		tooltip.title = ""; // ;-)
	}
	Utils.addEvent(target, "mouseover", function(ev) {
		return self._onMouseMove(ev);
	});
	if (Tooltip.prefs.move) {
		Utils.addEvent(target, "mousemove", function(ev) {
			return self._onMouseMove(ev);
		});
	}
	Utils.addEvent(target, "mouseout",  function(ev) {
		return self._onMouseOut(ev);
	});
	Utils.addEvent(tooltip, "mouseover", function(ev) {
		self.inTooltip = true;
	});
	Utils.addEvent(tooltip, "mouseout", function(ev) {
		ev || (ev = window.event);
		if (!Utils.isRelated(self.tooltip, ev)) {
			self.inTooltip = false;
			self.hide();
		}
	});
	self.wch = Utils.createWCH();
};
 ...
 */
Tooltip.setupFromDFN = function(class_re) {
	// init tooltips
...
Tooltip._C = null;	/**< [internal] keeps a reference to the currently displayed tooltip */
 ...
 * @param ev [Event] the event object
 */
Tooltip.prototype._onMouseMove = function(ev) {
	ev || (ev = window.event);
	if (this.timeout) {
		clearTimeout(this.timeout);
		this.timeout = null;
	}
	if (!this.visible && !Utils.isRelated(this.target, ev)) {
		var
			x = ev.clientX + 2,
			y = ev.clientY + 4;
		this.show(x, y);
...
Tooltip.prototype._onMouseOut = function(ev) {
	ev || (ev = window.event);
	var self = this;
	if (!Utils.isRelated(this.target, ev)) {
		if (this.timeout) {
			clearTimeout(this.timeout);
			this.timeout = null;
		}
...
Tooltip.prototype.show = function(x, y) {
	if (Tooltip._C) {
		if (Tooltip._C.timeout) {
			clearTimeout(Tooltip._C.timeout);
			Tooltip._C.timeout = null;
		}
		Tooltip._C.hide();
	}
	var t = this.tooltip;
	...
	Utils.fixBoxPosition(box);
	t.style.left = box.x + "px";
	t.style.top = box.y + "px";
	Utils.setupWCH_el(this.wch, t);
	Utils.addClass(this.target, "tooltip-hover");
	this.visible = true;
	Tooltip._C = this;
};
...
Tooltip.prototype.hide = function() {
	if (!this.inTooltip) {
		...
		Utils.hideWCH(this.wch);
		Utils.removeClass(this.target, "tooltip-hover");
		this.visible = false;
	}
};
