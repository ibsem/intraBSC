...
Utils.fixBoxPosition = function(box) {
	if (box.x < 0)
		box.x = 0;
	if (box.y < 0)
		box.y = 0;
	var cp = Utils.createElement("div");
	var s = cp.style;
	s.position = "absolute";
	s.right = s.bottom = s.width = s.height = "0px";
	window.document.body.appendChild(cp);
	var br = Utils.getAbsolutePos(cp);
...
Utils.isRelated = function (el, evt) {
...
Utils.getTargetElement = function(ev) {
	if (is_ie) {
		return window.event.srcElement;
	} else {
		return ev.target;
...
/// Removes an event handler added with Utils.removeEvent().  The
...
Utils.createElement = function(type, parent) {
	var el = null;
...
Utils.writeCookie = function(name, value, domain, path, exp_days) {
...
/** 
 * The reverse of Utils.makePref(), this function unserializes the
 ...
 */
Utils.mergeObjects = function(dest, src) {
	for (var i in src)
		dest[i] = src[i];
};
...
Utils.__wch_id = 0;	/**< [number, static] used to create ID-s for the WCH objects */
 ...
 */
Utils.setupWCH_el = function(f, el, el2) {
	if (f) {
		...
		if (el2) {
			var p2 = Utils.getAbsolutePos(el2),
				XX1 = p2.x,
				YY1 = p2.y,
		...
		Utils.setupWCH(f, X1, Y1, X2-X1, Y2-Y1);
	}
};
 ...
 */
Utils.setupWCH = function(f, x, y, w, h) {
	if (f) {
		var s = f.style;
 ...
 * @return false
 */
Utils.newCenteredWindow = function(url, windowName, width, height, scrollbars){
	var leftPosition = 0;
	var topPosition = 0;
	if (screen.width)
 ...
 */
Utils.selectOption = function(sel, val, call_default) {
 ...
 */
Utils.getNextSibling = function(el, tag) {
	el = el.nextSibling;
	if (!tag)
		return el;
	tag = tag.toLowerCase();
...
/** 
 * Similar to Utils.getNextSibling(), this function will return the
 ...
 * found.
 */
Utils.getFirstChild = function(el, tag) {
	el = el.firstChild;
	if (!tag)
		return el;
	tag = tag.toLowerCase();
	if (el.nodeType == 1 && el.tagName.toLowerCase() == tag)
		return el;
	return Utils.getNextSibling(el, tag);
};

Utils._ids = {};	/**< [number, static] maintains a list of generated IDs */
 ...
 */
Utils.generateID = function(code, id) {
...
Utils.addTooltip = function(target, tooltip) {
return new Tooltip(target, tooltip);
};
