		...
		<%if (lista != null){%>
			...
			<displaytag:table id="tabela" name="listaProcesso"    
				class="its" requestURI="" 
			 	...
			 	<c:if test="${tabela.ativo == 2}">
			 	<displaytag:column  style="width:16px;align:right;" >
						<img src="<c:out value="${base}"/>/WEB/imagens/comum/processoDetalhadoInativo.gif"
						 border="0" Title="Processo Detalhado Inativo"/>
				</displaytag:column>
				</c:if>
				
				<displaytag:column style="width:500px;align:left;" title="">
				...
				<displaytag:column  style="width:23px;align:left;" >
						...
						onclick="exportarProcesso(<c:out value='${tabela.id}'/>,'<c:out value='${tabela.nome}'/>');"
