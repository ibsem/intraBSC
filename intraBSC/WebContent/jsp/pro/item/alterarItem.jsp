...
<script language="JavaScript">
	<%java.util.Collection lista = (java.util.Collection) request.getSession().getAttribute("listaOpcaoItem");%>
	<%String codTipo = (String)request.getSession().getAttribute("codConfigTarefa");%>
	
	function voltarConfigTarefa(){
		document.itemForm.action = "<c:out value="${base}"/>/configTarefa/consultarUm.do?op=consultarUm&codigoConfigTarefa="+document.itemForm.codConfigTarefa.value;
		document.itemForm.submit();
	}
	
	function habilitaCombo(){
		if (document.itemForm.codigoDadoItem.value == "4"){
	...
	function setaValorSelecionado(args,args2){
		if (args2 != -1)
			document.itemForm.codigoSelecionadoExcluir.value = args.options[args2].value;
	}
	
	function excluirLista(){
		var codSel = document.itemForm.codigoSelecionadoExcluir.value;
		if (codSel != null){
			if(confirm("Deseja realmente excluir o textos do Combo?")){
				document.itemForm.action = "<c:out value="${base}"/>/listaOpcao/item.do?op=excluirListaOpcao&codListaOpcao="+codSel;
				document.itemForm.submit();
			}
		}else{
	...
	function incluirLista(){
		if (document.itemForm.nomeTextoCombo.value != ""){
			document.itemForm.action = "<c:out value="${base}"/>/listaOpcao/item.do?op=incluirListaOpcao&codigoDadoItem=4";
			document.itemForm.submit();
		}else{
	...
	function limparLista(){
		<%if (!lista.isEmpty()){%>
			if(confirm("Deseja realmente excluir todos os textos do Combo?")){
				document.itemForm.action = "<c:out value="${base}"/>/listaOpcao/item.do?op=limparListaOpcao";
				document.itemForm.submit();
			}
		<%}else{%>
	...
	function alterar(){
		...
		document.itemForm.action = "<c:out value="${base}"/>/item/alterar.do?op=alterar&tipoParticipacao=1&formAnce=configTarefaForm&acao=<c:out value="${base}"/>/configTarefa/consultarUm.do?op=consultarUm$codigoConfigTarefa="+codTipo;
		document.itemForm.submit();		
	}
	
</script>
<body onload="habilitaCombo();"></body>
<html:form action="/item/alterar" onsubmit="return validateItemForm(this);">
		...
		<table border="0" width="380px" cellpadding="0" cellspacing="0">
			...
			<tr>	
				<td widht="50%">
					<input type="hidden" name="numOrdemAnterior" value="<c:out value="${itemForm.numeroOrdem}"/>">
					<html:text property="numeroOrdem" size="21" disabled="false" maxlength="3" />
				</td>
				<td widht="30%">					
					<html:select property="indItemVigente" >
				...
				<td widht="20%" align="right" >
					<html:select property="codigoDadoItem" onchange="habilitaCombo();">
		...
		<table border="0" width="385px" cellpadding="0" cellspacing="0">			
				<tr>
					<td colspan="2"><bean:message key="pro.label.item.opcoes.combo" /></td>
				</tr>
				<tr>
					<td colspan="2">
				...
				<tr>
					<td colspan="2">
						<html:select property="listaTextoCombo"   size="2" style="width: 380px;height: 80px;" onclick="javascript: setaValorSelecionado(this,this.selectedIndex);">
							<logic:present name="listaOpcaoItem">
								<html:options collection="listaOpcaoItem" property="codigo" labelProperty="texto" />
							</logic:present>
						</html:select>
					</td>
				</tr>
